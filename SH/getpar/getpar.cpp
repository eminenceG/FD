 ...
 *
 * DESCRIPTION
 *	retrieve command line arguments.  Acquired from Caltech.
 *
...
int gp_getpar_err(char *subname,char *mess,...);
...
setpar(int ac,char **av)		/* set up arglist & process INPUT command */
	...
	*value= '\0';
	...
	addflags= 0;
	*value= '\0';
	if(GETPAR("STOP","b",(int *) value)) addflags |= STOP;
	*value= '\0';
	if(GETPAR("VERBOSE","b",(int *) value)) addflags |= VERBOSE;
	*value= '\0';
#ifdef FORTRAN
	if(GETPAR("LIST","s",(int *) value, 0, 0, 0))
#else
	...
	*value= '\0';
#ifdef FORTRAN
	if(GETPAR("INPUT","s",(int *) value, 0, 0, 0))
#else
...
void gp_add_entry(char *name,char *value)	/* add an entry to arglist, expanding memory */
   {
	struct arglist *alptr;
	int len;
	register char *ptr;
	...
	/* check argbuf memory */
	len=(int) (strlen(name) + strlen(value) + 2); /* +2 for terminating nulls */
	if(NBUF+len >= BUFMAX)
	   {
		BUFMAX += BUFINC;
		if(ARGBUF == NULL)
	...
	/* add value */
	NBUF += len;
	alptr->argval_offset= (int) (ptr - ARGBUF);
...
gp_do_environment(int ac,char **av)
   {
	char **ae;
	register char *pl, *pn, *pv;
...
int
#ifdef FORTRAN
mstpar_(name,type,val,dum1,dum2,lens)
int dum1, dum2, lens;	/* dum1 & dum2 are extra args that fortran puts in */
#else
mstpar(char *name,char *type,const void *valval)
#endif
   {
	int cnt;
	char *typemess;
...
int
#ifdef FORTRAN
getpar_(name,type,val,dum1,dum2,lens)
int dum1, dum2, lens;	/* dum1 & dum2 are extra args that fortran puts in */
#else
getpar(char *name,char *type,const void *valval)
#endif
   {
	int *val=(int*)valval;
	register char *sptr;
	...
	if(*pl == '(' )
	{
		...
		*pv = '\0';
		if(*pl == ')' )
		{
			...
			ARGHEAD= ARGLIST;
#ifndef	FORTRAN
			if(GETPAR(subname, "s", (int *) pv))
#else
			if(GETPAR(subname, "s", (int *) pv, 0, 0, 0))
#endif
			{
				pv += strlen(pv);
			}
...
#ifndef FORTRAN
char *
mstspar(char *parname)
{
	char parvalue[MAXVALUE];
	
	mstpar(parname, "s", (int *) parvalue);
	
	return(strdup(parvalue));
}
char *
getspar(char *parname, char *defvalue)
{
	char parvalue[MAXVALUE];
